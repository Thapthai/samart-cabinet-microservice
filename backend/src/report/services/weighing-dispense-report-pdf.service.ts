...
@Injectable()
export class WeighingDispenseReportPdfService {
  ...
  async generateReport(data: WeighingDispenseReportData): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        doc.text('รายการเบิกอุปกรณ์จากตู้ Weighing', margin, headerTop + 6, { width: contentWidth, align: 'center' });
        doc.fontSize(11).font(finalFontName).fillColor('#6C757D');
        doc.text('Weighing Dispense Report', margin, headerTop + 22, { width: contentWidth, align: 'center' });
